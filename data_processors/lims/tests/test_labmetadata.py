...
def _generate_labmetadata_row_dict(id: str) -> dict:
    ...
    :param id: this just gets used as a suffix
    :return: row dict
    """
...
class LimsUnitTests(LimsUnitTestCase):
    ...
    def test_labmetadata_non_nullable_columns(self) -> None:
        ...
        process_results = persist_labmetadata(_generate_labmetadata_df([row_1, row_2, row_3, row_4, row_5, row_6, row_6]))
        
        self.assertEqual(process_results['labmetadata_row_update_count'], 0)
        self.assertEqual(process_results['labmetadata_row_new_count'], 2) # row_2 and one of row_6 duplicates
        self.assertEqual(process_results['labmetadata_row_invalid_count'], 5)
...
class LimsIntegrationTests(LimsIntegrationTestCase):
    ...
    def test_scheduled_update_handler(self):
        """
        python manage.py test data_processors.lims.tests.test_labmetadata.LimsIntegrationTests.test_scheduled_update_handler
        """
        result = labmetadata.scheduled_update_handler({'event': "LimsIntegrationTests lims update event"}, None)

        logger.info("-" * 32)
        logger.info("Example LimsIntegrationTests.scheduled_update_handler lambda output:")
        logger.info(json.dumps(result))
